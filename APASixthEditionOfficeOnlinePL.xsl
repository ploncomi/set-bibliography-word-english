...
<!-- Store this file in C:\Users\user\AppData\Roaming\Microsoft\Bibliography\Style -->
<!-- Created by Patricio Loncomilla -->
      ...
      <xsl:when test="b:StyleNameLocalized">
        <xsl:choose>
          ...
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1087'">
            <xsl:text>Американдық психологиялық қауымдастық PL</xsl:text>  <!-- PL -->
          </xsl:when>
          ...
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1060'">
            <xsl:text>Standard APA PL</xsl:text>  <!-- PL -->
          </xsl:when>
        </xsl:choose>
      </xsl:when>
				...
				<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40">
					...
					<body>
						...
						<xsl:if test="string-length($year) > 0">
							<xsl:text> </xsl:text>
						    <xsl:call-template name="templ_prop_OpenBracket"/>   <!-- PL -->
							<xsl:value-of select="$year"/>  <!-- PL -->
							<!--
